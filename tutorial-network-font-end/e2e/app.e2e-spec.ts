...
describe('Starting tests for tutorial-network-font-end', function() {
  ...
  it('website title should be tutorial-network-font-end', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorial-network-font-end');
    })
  });
  ...
  it('navbar-brand should be tutorial-network-font-end',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial-network-font-end');
    });
  });
